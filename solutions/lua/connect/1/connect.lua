...
return {
  winner = function(board_str)
		...
		for i = 1, board.height do
			if find_path(board, {}, i, 1, "X", left_right) then
				return 'X'
			elseif find_path(board, {}, i, 1, "O", left_right) then
				return 'O'
			end
		end
		for j = 1, board.width do
			if find_path(board, {}, 1, j, "X", top_bottom) then
				return "X"
			elseif find_path(board, {}, 1, j, "O", top_bottom) then
				return "O"
			end
		end
		return ''
