...
std::string to_string(Nationality nationality) {
	switch (nationality) {
...
bool is_valid(const PartialState& state) {
	for (int i = 0; i < 5; i++) {
...
inline bool find(PartialState&, int) {
	return true;
}

template<bool other(PartialState& state, int i), typename T, std::optional<T> House::*Member>
bool find(PartialState& state, int i) {
	auto get_field = [&](int k) -> std::optional<T>& {
		return state[k].*Member;
	};
	for (auto n = 0ul; n < 5ul; n++) {
		const auto elt = static_cast<T>(n);
		for (auto j = 0ul; j < (std::size_t)i; j++)
		...
		if (not is_valid(state))
			continue;
		if (i == 4) {
			if (not other(state, 0))
				continue;
			else
				return true;
		}
		if (not find<other, T, Member>(state, i+1))
				continue;
		else
			return true;
	next:
