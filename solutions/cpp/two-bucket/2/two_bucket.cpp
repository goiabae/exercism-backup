...
bool aux(
	...
	for (auto action : actions) {
		...
		{
			action_chain.push_back(action);
			if (new_buckets[0].volume == target_volume or new_buckets[1].volume == target_volume)
				return true;

			auto result = aux(action_chain, states, new_buckets, target_volume, start_bucket, other_bucket);
			if (result) return true;
			action_chain.pop_back();
		}
	}
	return {};
}
...
measure_result measure(
	...
	int other_index = (start_index + 1) & 1;

	auto found_chain = aux(action_chain, states, start_buckets, target_volume, start_index, other_index);
	...
	auto num_moves = (int)action_chain.size();
