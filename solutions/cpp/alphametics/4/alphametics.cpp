...
auto solve_aux(const std::vector<Equation>& eqns, std::vector<varptr>& variables, int start, const std::vector<std::vector<varptr>>& number_digits, std::bitset<10>& used_numbers) -> Equation::State {
	for (auto j = 0; j < 10; j++) {
		if (used_numbers[j]) continue;
		used_numbers[j] = true;
		*variables[start] = j;
		if (static_cast<std::size_t>(start) == variables.size()-1) {
			const auto all_solved = std::all_of(eqns.cbegin(), eqns.cend(), [](const Equation& eqn) -> bool{
