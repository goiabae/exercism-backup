...
auto solve_aux(const std::vector<Equation>& eqns, std::vector<varptr>& variables, int start, const std::vector<std::vector<varptr>>& number_digits, std::bitset<10>& used_numbers, const std::vector<bool>& leading) -> bool {
	for (auto j = 0 + leading[start]; j < 10; j++) {
		...
		*variables[start] = j;
		if (static_cast<std::size_t>(start) == variables.size()-1) {
			...
			const auto res = solve_aux(eqns, variables, start+1, number_digits, used_numbers, leading);
			if (res) return true;
		}
...
std::optional<std::map<char, int>> solve(std::string input) {
	auto p = input.find("==");
	const auto operands = split_on_pluses(input.substr(0, p-1));
	const auto result = input.substr(p+3);
	std::vector<varptr> variables {};
	std::array<varptr, 26> char_to_variable {};
	char_to_variable.fill(nullptr);
	{
		for (auto i = 0ul; i < operands.size(); i++) {
			for (const auto& c : operands.at(i)) {
				if (char_to_variable[c - 'A'] == nullptr) {
					auto p = new int {-1};
					char_to_variable.at(c - 'A') = p;
	...
	std::vector<bool> leading(variables.size(), false);
	for (const auto& operand : operands) {
		auto c = operand[0];
		if (char_to_variable[c - 'A'] != nullptr) {
			for (auto i = 0ul; i < variables.size(); i++) {
				if(variables[i] == char_to_variable[c - 'A']) {
					leading[i] = true;
					break;
				}
			}
	...
	const auto equation_count = [&](){
		auto w = 0ul;
		for (const auto& opnd : operands)
			if (opnd.size() > w) w = std::max(w, opnd.size());
		w = std::max(w, result.size());
		return w;
	}();
	if (result.size() < equation_count) return {};
	...
	std::vector<std::vector<varptr>> number_digits {};
	{
		for (const auto& operand : operands) {
			...
			for (const auto& c : operand) {
				digits.push_back(char_to_variable.at(c - 'A'));
			}
	...
	for (auto i = (int)equation_count-1; i >= 0; i--) {
		Equation eqn {};
		if (i == (int)equation_count-1) {
			eqn.previous_equation = nullptr;
		} else {
			eqn.previous_equation = &eqns.back();
		}
		eqn.expected_result = char_to_variable.at(result[i] - 'A');
		eqn.summands = {};
		for (auto j = 0ul; j < padded_operands.size(); j++) {
			const auto& padded_operand = padded_operands[j];
			const auto& column = padded_operand[i];
			if (column != ' ') {
				eqn.summands.push_back(char_to_variable.at(column - 'A'));
			}
		}
		eqns.push_back(eqn);
	}
	std::bitset<10> used_numbers {};
	auto solution_found = solve_aux(eqns, variables, 0, number_digits, used_numbers, leading);
	if (solution_found) {
		std::map<char, int> result {};
		for (auto i = 0ul; i < 26; i++) {
			if (auto p = char_to_variable.at(i); p != nullptr) {
				const auto n = *p;
				result['A' + i] = n;
			}
		}
